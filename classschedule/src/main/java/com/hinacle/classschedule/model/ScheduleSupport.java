...
public class ScheduleSupport {
     ...
     */
    public static List<String> getDateStringFromWeek(Long start ,int curWeek, int targetWeek) {
     ...
     * @return 8个元素的集合，第一个为月份，之后7个依次为周一-周日
     */
    public static List<String> getWeekDate(Long start) {
...
//            if(is) result.add(s);
//        }
        List<Schedule> list = new ArrayList<>(data);
        sortList(list);
        return list;
    }
     ...
     * @return
     */
    public static List<Schedule> flitSchedule(List<Schedule> data, int curWeek, boolean isShowNotCurWeek) {
        if (data == null) return new ArrayList<>();
        Set<Schedule> result = new HashSet<>();
